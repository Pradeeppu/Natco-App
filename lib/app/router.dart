...
import 'package:natco_app/features/assessments/presentation/screen/assessment_form_screen.dart';
...
import 'package:natco_app/features/schools/presentation/screen/school_form_screen.dart';
...
import 'package:natco_app/features/students/presentation/screen/student_form_screen.dart';
import 'package:natco_app/features/students/presentation/screen/student_import_screen.dart';
...
import 'package:natco_app/features/users/presentation/screen/user_detail_screen.dart';
import 'package:natco_app/features/users/presentation/screen/user_form_screen.dart';
import 'package:natco_app/features/users/presentation/screen/users_screen.dart';
...
final List<GuardedRoute> kAppRoutes = <GuardedRoute>[
  ...
  GuardedRoute(
    path: RoutePaths.schoolNew,
    rule: const RouteAccessRule.requires(Permission.manageSchools),
    builder: (_, _) => const SchoolFormScreen(),
  ),
  GuardedRoute(
  ...
  GuardedRoute(
    path: RoutePaths.studentNew,
    rule: const RouteAccessRule.requires(Permission.manageStudents),
    builder: (_, _) => const StudentFormScreen(),
  ),
  GuardedRoute(
    path: RoutePaths.studentImport,
    rule: const RouteAccessRule.requires(Permission.importStudents),
    builder: (_, _) => const StudentImportScreen(),
  ),
  GuardedRoute(
  ...
  GuardedRoute(
    ...
    rule: const RouteAccessRule.requires(Permission.viewUsers),
    insideShell: true,
    builder: (_, _) => const UsersScreen(),
  ),
  GuardedRoute(
    path: RoutePaths.userNew,
    rule: const RouteAccessRule.requires(Permission.manageUsers),
  ...
  GuardedRoute(
    path: RoutePaths.assessmentNew,
    rule: const RouteAccessRule.requires(Permission.manageAssessments),
  ...
  GuardedRoute(
    ...
    builder: (_, GoRouterState state) =>
        AnswerKeyScreen(assessmentId: state.pathParameters['assessmentId']!),
  ),
  GuardedRoute(
    path: RoutePaths.assessmentSession,
    rule: const RouteAccessRule.requires(Permission.conductAssessment),
    builder: (_, GoRouterState state) =>
        AssessmentSessionScreen(sessionId: state.pathParameters['sessionId']!),
  ),
  GuardedRoute(
  ...
  GuardedRoute(
    ...
    rule: const RouteAccessRule.requires(Permission.validateOmr),
    builder: (_, GoRouterState state) =>
        OmrValidationScreen(omrId: state.pathParameters['omrId']!),
  ),
  GuardedRoute(
  ...
  GuardedRoute(
    ...
    insideShell: true,
    builder: (_, GoRouterState state) =>
        StudentResultScreen(studentId: state.pathParameters['studentId']!),
  ),
  GuardedRoute(
...
final Provider<GoRouter> routerProvider = Provider<GoRouter>((Ref ref) {
  ...
  // Ordering matters, and not in the way the route table's own order
  // suggests. go_router takes the first route that matches, and the create
  // forms (`/schools/new`) sit *outside* the shell while the detail screens
  // (`/schools/:schoolId`) sit inside it — a form is a task, not a
  // destination. Emitting the shell first therefore let `/schools/:schoolId`
  // swallow `/schools/new` and render the detail screen for a school whose id
  // was the string "new": a "could not be found" page where the Add button
  // should have been, with nothing in the logs pointing at routing.
  //
  // Literal segments are more specific than parameters, so the routes that
  // contain no parameter are offered first. `test/app/literal_route_
  // precedence_test.dart` pins this, because the ordering that makes it work
  // is not visible in the route table anyone reads.
  int parameterCount(GuardedRoute route) => ':'.allMatches(route.path).length;
  int bySpecificity(GuardedRoute a, GuardedRoute b) =>
      parameterCount(a).compareTo(parameterCount(b));

  final List<GuardedRoute> orderedTopLevel =
      List<GuardedRoute>.of(topLevelRoutes)..sort(bySpecificity);
  final List<GuardedRoute> orderedShell = List<GuardedRoute>.of(shellRoutes)
    ..sort(bySpecificity);
  ...
  return GoRouter(
    ...
    routes: <RouteBase>[
      // Top level before the shell: see the ordering note above.
      ...orderedTopLevel.map(
        (GuardedRoute route) =>
