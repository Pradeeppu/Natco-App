...
`pubspec.yaml` are the source of truth; this document explains *why*
...
| `csv` | report export, CSV student import | §32 requires CSV first; §11 requires bulk student import. |
| `file_picker` | CSV student import | Actively maintained, MIT, reads an Android content-URI via SAF (`withData: true` returns bytes directly, since a raw path is not reliably readable from a `content://` URI). Added in phase 2 — evaluated against the seven checks in §7: no extra Android permission beyond what SAF itself grants per-pick, works offline, and does exactly the one thing needed (pick one file, get its bytes). |
