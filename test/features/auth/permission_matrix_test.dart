...
void main() {
  ...
  group('permission matrix', () {
    ...
    test('a PST Teacher enters students but cannot bulk-import them', () {
      // Single entry and bulk load are separate permissions on purpose: a
      // mis-mapped CSV column is a whole-class error, a mistyped name is one.
      expect(UserRole.pstTeacher.can(Permission.manageStudents), isTrue);
      expect(UserRole.pstTeacher.can(Permission.importStudents), isFalse);
    });
  ...
  group('critical restrictions', () {
    ...
    test('only Super Admin may change system configuration', () {
      expect(_rolesWith(Permission.manageSystemConfig), <UserRole>{
        UserRole.superAdmin,
      });
    });

    test('user management is limited to Super Admin and Supervisor', () {
    ...
    test('a Supervisor may create PST Teachers and nothing else', () {
      // `manageUsers` alone would let a Supervisor mint a Super Admin — an
      // escalation no scope check catches, because a Super Admin has no scope
      // to be outside of.
      expect(UserRole.supervisor.assignableRoles, <UserRole>{
        UserRole.pstTeacher,
      });
      expect(UserRole.supervisor.canAssign(UserRole.superAdmin), isFalse);
      expect(UserRole.supervisor.canAssign(UserRole.supervisor), isFalse);
      expect(UserRole.supervisor.canAssign(UserRole.assessmentAdmin), isFalse);
      expect(UserRole.supervisor.canAssign(UserRole.scannerOperator), isFalse);
    });

    test('Super Admin may assign every role', () {
      expect(UserRole.superAdmin.assignableRoles, UserRole.values.toSet());
    });

    test('a role without manageUsers can assign nothing', () {
      // Otherwise the two halves could disagree, and the half that is only
      // checked in the UI is the half that gets bypassed.
      for (final UserRole role in UserRole.values) {
        if (!role.can(Permission.manageUsers)) {
          expect(
            role.assignableRoles,
            isEmpty,
            reason: '${role.wireName} can assign roles it cannot manage',
          );
        }
      }
    });
    ...
    test('the geographic hierarchy is edited by Super Admin alone', () {
      // Students are the exception below: a school's shape is a decision about
      // the programme, a student is a decision about a classroom.
      for (final Permission permission in <Permission>[
    ...
    test('student records are edited by Super Admin and the class teacher', () {
      expect(_rolesWith(Permission.manageStudents), <UserRole>{
        UserRole.superAdmin,
        UserRole.pstTeacher,
      });
      // A Supervisor monitors and validates; they do not edit the register.
      expect(UserRole.supervisor.can(Permission.manageStudents), isFalse);
      expect(UserRole.viewer.can(Permission.manageStudents), isFalse);
    });
  });
